...
* **타임라인을 통째로 다시 그리지 않는다.** 서버는 HTML 을 한 번만 주고, 이후엔
  SSE 로 온 JSON 한 건마다 노드를 만들어 붙이기만 한다. 이미 있는 노드는 다시
  만들지도 지우지도 않는다(`innerHTML` 을 쓰지 않는다 — 덤으로 XSS 봉쇄).
...
* **가상 스크롤이 없다.** 위로 스크롤하면 과거가 자동으로 이어 붙고(한 쪽
  50건, gitwire 의 keyset 페이징이라 요청한 만큼만 읽는다) 맨 위에 닿으면
  멈춘다. 다만 붙은 노드를 다시 걷어내지는 않으므로, 수천 건까지 계속 거슬러
  올라가면 그만큼 DOM 이 커진다.
