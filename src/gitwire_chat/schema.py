...
* ``Record.sender`` — **어느 참가자 프로세스가 발행했나**(전송 수준 식별자,
  IP 주소에 가깝다).
  → 표시 이름으로 쓰지 **않는다.** 다만 "내가 보낸 것인가"(화면의 좌우 배치,
    로컬 에코 판정)와 같은 기계 판단에는 쓴다 — 그 비교를 하는 곳은
    ``rooms.RoomManager.is_mine`` **한 곳**이고, 이 모듈은 봉투를 그대로
    옮기기만 한다(누가 '나'인지는 이 모듈이 알 수 없다).
...
class Message:
    ...
    ``mine`` 을 여기 담지 않는 이유: '내 것'은 봉투의 속성이 아니라 **읽는
    쪽과의 관계**다. 같은 레코드가 내 화면에서는 내 것이고 상대 화면에서는
    남의 것이다. 그래서 판정은 설치본 식별자를 아는 ``RoomManager`` 가 하고,
    이 모듈은 관계를 모른다."""
