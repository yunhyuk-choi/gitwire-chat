...
class RoomManager:
    ...
    def _open_channel(self, room: Room):
        ...
        kwargs: dict[str, Any] = {
            ...
            # `_after_push()` 가 아예 돌지 않는다 → **내가 보낸 말의 로컬 에코가
            # 나가지 않는다** (`_deliver(own=True)`): 브라우저의 낙관적 항목이
            # 진짜 봉투로 갈아끼워지지 않아 *전송 중* 으로 남고, 탭이 없으면
            # 알림 경로도 그 자리에서 끊긴다.
            #
    ...
    def _after_push(self, room_id: str, records: list) -> None:
        """레코드가 원격에 **착지했다** — 화면에 흘린다.

        ⭐⚠️ **여기서 읽음 커서를 올리지 않는다.** 예전에는 `mark(records[-1].id)`
        로 내 커서를 내 메시지까지 올렸다. 그 전진은 아무것도 사지 못했다:

        * 카운트 공식은 **작성자를 분모에서 뺀다**(`p ≠ A` — `reads` 모듈 도크).
          내 커서 값은 내 메시지의 카운트에 영향이 없다.
        * 아카이브 합의도 커서를 안 본다 (`reads.consensus_day` 는 `archived`
          워터마크만 본다).
        * 뱃지가 내 말을 세던 것은 커서 문제가 아니라 세는 루프가 작성자를 안
          가린 문제였고, 이제 그 루프가 가린다 (`reads.ReadTracker.unread`).

        값은 왕복으로 치렀다 — 전진이 참가자 상태 파일을 다시 쓰게 만들어
        **전송당 커밋·push 가 하나 더** 나갔고(실측: 「1 record(s)」 뒤 2초쯤 뒤
        「참가자 상태 1건」이 예외 없이 따라왔다), 그 두 번째 push 가 다음 전송의
        레코드 스탬프를 439ms 늦췄다. 그래서 **보내기 경로에는 커서 발행 트리거가
        없다.** 커서가 오르는 자리는 "보이는 탭에서 남의 안 읽은 말을 실제로
        읽었을 때" 하나다 (`mark_read` ← `static/js/reads.js`).

        ⭐ 남는 것은 **상태 축**뿐이다: 말을 보낸 사람이 "메시지를 읽을 수 없는
        상태"일 수는 없으므로 `활동 중` 을 *선언한다*. 이미 활동 중이면
        `publish` 의 가드가 아무것도 쓰지 않으므로(= 커밋이 생기지 않으므로)
        평상시 전송은 **커밋 하나·push 하나**로 끝난다. `자리 비움`·`방해 금지`
        였다가 말을 보낸 그 한 번만 파일이 바뀌고, 그것은 실제로 바뀐 사실이다.
        """
        try:
            tracker = self.reads(room_id)
            # ⚠️ 커서를 주지 않는다 (위 도크). 상태만 선언한다.
            if tracker.publish(status=_reads.STATUS_ACTIVE):
                self._nudge_outbox(room_id)   # 실제로 썼을 때만 밀어낸다
        except Exception:  # noqa: BLE001 — 읽음 표시가 전달을 막지 않는다
            log.debug("방 %s push 후 가용 상태 발행 실패", room_id, exc_info=True)
