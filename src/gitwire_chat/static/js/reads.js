 ...
 *
 * ⭐ 커서 전진의 기준 — "보이는 탭에서 화면에 들어온 **남의** 최대 메시지"
 * -------------------------------------------------------------------
 ...
 *
 * ⭐⚠️ 그 최대값에서 **내 메시지는 빠진다** (`isMine` — 아래 `participantOf` 와
 * 같은 규칙이다). 내가 보낸 말이 창 안에 들어오면 그것이 곧 최대값이라, 전송마다
 * 커서가 전진하고 발행이 나갔다 — 그것이 **전송당 두 번째 push** 의 정체였다
 * (실측: 「1 record(s)」 뒤 2초쯤 뒤 「참가자 상태 1건」, 그 push 가 다음 전송의
 * 레코드 스탬프를 439ms 늦췄다). 내 커서 값은 내 메시지의 카운트에 아무 영향이
 * 없고(공식이 작성자를 뺀다) 뱃지도 이제 내 레코드를 세지 않으므로
 * (`gitwire_chat/reads.py`), 그 발행은 살 것이 없는 왕복이었다.
 *
 * 그래서 발행 조건은 둘의 **곱**이다: *내가 아직 읽지 않은 남의 메시지가 있다*
 * (= 남의 ID 가 내 커서보다 뒤다) **그리고** *보이는 탭이다* (`visible()`).
 * "새로운"은 도착 시각이 아니라 **내가 아직 안 읽음**이라는 뜻이다 — 이미 있던
 * 남의 과거 메시지를 뒤늦게 처음 읽는 경우도 그대로 발행된다.
 *
 ...
 *
 * ⚠️ 그 "유일함"은 **공식**에 대한 것이다. 판정 1(작성자 제외)은 서버의 뱃지도
 * 쓴다 (`gitwire_chat/reads.py` 의 `unread` — 거기서는 봉투 대신 레코드 ID 에
 * 박힌 발신자로 같은 짝짓기를 한다). 값이 다른 두 파생값이 같은 판정을 공유하는
 * 것이고, 그 둘이 어긋나면 "내 뱃지에는 안 뜨는데 상대 화면에서는 내가 안 읽은
 * 사람"이 된다 — 그래서 **양쪽을 테스트가 같은 값으로 묶어 둔다**
 * (`tests/test_reads.py` · 아래 `isMine` 의 화면 쪽 검증).
 *
...
/* ⭐ **이 메시지를 내가 썼나** — 작성자 판정의 세 번째 소비자다. 규칙은 위
 * `participantOf` 하나이고 여기서 다시 쓰지 않는다(카운트 공식의 1번과 같은 것):
 * 봉투에는 사람 키가 없고 설치본 식별자(`sender`)만 있으므로 참가자 파일의
 * `senders` 목록으로 짝짓고, 그 참가자가 **나**(`me`)인지 본다.
 *
 * 쓰는 곳은 커서 전진의 문이다 (`timeline.js` 가 창 안의 최대값을 고를 때).
 * 내 것을 빼야 전송이 커서를 밀지 않는다 (모듈 도크).
 *
 * ⚠️ **모르면 `false`** 다. 아직 참가자 지도를 못 받은 순간(방을 막 열었다)에
 * `true` 로 기울면 남의 새 메시지를 내 것으로 보아 **읽음이 영원히 안 나간다** —
 * 조용히 "다 읽음"이 되는 쪽이고, 이 파일이 일관되게 피하는 방향이다. 반대로
 * 기울면 그 짧은 창에서 발행 한 번이 더 나갈 뿐이다.
 */
export function isMine(me, participants, msg) {
  if (!me || !msg) { return false; }
  var who = participantOf(participants, msg.sender);
  return !!who && who.key === me;
}
...
export function createReads(env) {
  ...
  /* 커서 전진의 문 — 타임라인이 창 안에서 **남의** 최대 ID 만 고르게 해 준다.
     판정은 모델을 가진 우리가 하고(`isMine`), 창 내용을 아는 것은 그쪽이다. */
  function mine(msg) {
    return isMine(model.me, model.participants, msg);
  }
